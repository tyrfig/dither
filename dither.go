...
func NewDitherer(palette []color.Color) *Ditherer {
	...
	for i := range d.linearPalette {
		...
		d.labPalette[i] = [3]float64{L, labA, labB}
...
func (d *Ditherer) closestColorAND(r, g, b uint16) int { // closestColorAND
	// Go through each color and find the closest one

	labL, labA, labB := xyz2lab(linearRGBtoXYZ(r,g,b))
	
	color, best := 0, uint32(math.MaxUint32)
	colorLABbw, bestLABbw := 0, float64(100000000.0) // includes black and white
	colorLAB, bestLAB := 0, float64(100000000.0)
	...
	for i, c := range d.linearPalette {
		...
		deltaB := labB - cB
                ...
                distLAB := math.Sqrt(deltaL * deltaL + deltaA * deltaA + deltaB * deltaB)
	...
	if (c[0] == c[1] && c[0] == c[2] && colorLABbw == color) {
	    return color // black, white or gray
	}
	return colorLAB
}
...
func (d *Ditherer) DitherAND(src image.Image) image.Image {
	if d.invalid() {
		panic("dither: invalid Ditherer")
	}
	...
	if d.Mapper != nil {
		...
		parallel(workers, img.(draw.Image), img, func(x, y int, c color.Color) color.Color {
			...
			return d.premult(
				...
				// -> cast to color.RGBA64
				// Comes from d.palette so this cast will always work
				d.palette[d.closestColorAND(d.Mapper(x, y, r, g, b))].(color.RGBA64),
				x, y, img,
			)
		})
		return img
	...
	for y := b.Min.Y; y < b.Max.Y; y++ {
		for x := b.Min.X; x < b.Max.X; x++ {
			...
			// Quantize current pixel
			oldR, oldG, oldB := linearAt(x, y)
			newColorIdx := d.closestColorAND(oldR, oldG, oldB)
			img.Set(x, y, d.premult(d.palette[newColorIdx].(color.RGBA64), x, y, img))
